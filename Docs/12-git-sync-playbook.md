...
## Быстрый промпт для синхронизации

Скопировать и отправить Codex, когда нужно получить последние файлы из `origin/main`:

```text
Кодекс, подтяни последние изменения из origin/main. Сначала проверь git status; если есть локальные изменения — остановись и покажи их, если чисто — сделай checkout main и pull.
```
