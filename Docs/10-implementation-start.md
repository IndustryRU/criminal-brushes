...
Дата первоначальной проверки: 2026-06-30.

Актуализация: 2026-07-14.

## Текущий статус

- Git и `origin/main` настроены.
- FVM `4.1.2` установлен, Flutter закреплен на `3.44.4`.
- Вместе с Flutter используется Dart `3.12.2`.
- Flutter app создан в `apps/customer_app` для Web, Android и iOS.
- Supabase CLI `2.109.1` привязан к `criminal-brushes-dev`.
- Локальная и удаленная история начальной миграции синхронизированы.
- GitHub Actions выполняет format, analyze, test и release web build.

Разделы ниже сохранены как история первоначального аудита и инструкция восстановления окружения.
