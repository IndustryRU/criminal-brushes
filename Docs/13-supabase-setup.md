...
## 5. Привязка Supabase CLI к облачному проекту
...
Docker для этого не нужен. Он требуется только для локального `supabase start`, который сейчас не является обязательным процессом проекта.
...
Если CLI не получается использовать, можно применить SQL через `Supabase Dashboard -> SQL Editor`, но файл миграции все равно должен быть сохранен в Git.
...
Студенту для полноценной учебной работы нужны:

- доступ к GitHub repository;
- Supabase project access к `criminal-brushes-dev` с минимальной подходящей ролью;
- `SUPABASE_URL`;
- `SUPABASE_ANON_KEY`;
...
После приглашения студент может сам взять `SUPABASE_URL` и `SUPABASE_ANON_KEY` в `Project Settings -> API`.
...
1. Заполнить локальный `.env.dev` по `.env.example`.
